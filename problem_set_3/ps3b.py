...
class Patient(object):
    ...
    def update(self):
        ...
        for i in self.getViruses():
            try:
                array.append(virus.reproduce(popDensity))
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb, numTrials):
    ...
    timesteps = 300
    trials = [[] for s in range(timesteps)]
    for x in range(numTrials):
        viruses = [SimpleVirus(maxBirthProb, clearProb) for v in range(numViruses)]
        patient = Patient(viruses, maxPop)
        for step in range(timesteps):
            trials[step].append(patient.update())
    results = [sum(l) / float(numTrials) for l in trials]
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        if (all(self.isResistantTo(i) for i in activeDrugs)) and random.random() < self.maxBirthProb * (1 - popDensity):
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances, mutProb, numTrials):
    ...
    maxBirthProb: Maximum reproduction probability (a float between 0-1)
    clearProb: maximum clearance probability (a float between 0-1)
    resistances: a dictionary of drugs that each ResistantVirus is resistant to (e.g., {'guttagonol': False})
    ...
    timesteps = 300
    treatment = 150
    total = [[] for x in range(timesteps)]
    resistant = [[] for x in range(timesteps)]
    ...
    for t in range(numTrials):
        ...
        for step in range(timesteps):
            ...
            patient.update()
            total[step].append(patient.getTotalPop())
            resistant[step].append(patient.getResistPop(['guttagonol']))
    resultsTotal = [sum(l) / float(numTrials) for l in total]
    resultsResistant = [sum(l) / float(numTrials) for l in resistant]

    pylab.plot(resultsTotal, label="Total Virus Population")
    pylab.plot(resultsResistant, label="Resistant Virus Population")
